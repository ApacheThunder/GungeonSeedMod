...
namespace DungeonSeedMod {

    public class DungeonSeedMod : ETGModule {

        public override void Init() { }
        public override void Exit() { }

        public override void Start() {
            ETGModConsole.Commands.AddGroup("dungeonseed", delegate (string[] e) {
                ETGModConsole.Log("[Dungeon Seed]  The following options are available for setting Dungeon Seeds:");
                ETGModConsole.Log("checkseed\nsetseed\nnewseed\nreset\n\nTo set a custom seed, use 'setseed SEED1 SEED2' where SEED1 is the main Dungeon seed between 1 and 1000000000 and\n SEED2 is the RandomIntForRun seed. Set Seed2 to a value of 0 to 1000.\nNote that setting SEED1 to zero will disable seeded runs.\nTo ensure consistant results, set your seed after selecting a character at the breach but before leaving the breach.\nSetting a seed while in the middle of a run or via quick restart is not recommended.\nTo start a new seed, simply use 'dungeonseed newseed' and write down the displayed seed if you want to share it!.\nUse 'dungeonseed checkseed' to check your current seed.\n\nTo turn off custom seed mode, use 'dungeonseed reset' or set SEED1 argument of setseed to zero.");
            });
            ETGModConsole.Commands.GetGroup("dungeonseed").AddUnit("checkseed", delegate (string[] e) { InitializeSeed(CheckSeed: true); });
            ETGModConsole.Commands.GetGroup("dungeonseed").AddUnit("setseed", SetSeed);
            ETGModConsole.Commands.GetGroup("dungeonseed").AddUnit("newseed", delegate (string[] e) {
                InitializeSeed(BraveRandom.GenerationRandomRange(1, 1000000000), Random.Range(0, 1000), false);
            });
            ETGModConsole.Commands.GetGroup("dungeonseed").AddUnit("reset", delegate (string[] e) {
        ...
        private void SetSeed(string[] args)
        {
            ...
            if (int.Parse(args[0]) > 1000)
            {
                ETGModConsole.Log("Error: Requested RandomIntForRun seed exceeds normal maximum value of 1000");
                return;
            }

            if (int.Parse(args[0]) < 1) { ETGModConsole.Log("Warning: Setting a seed of 0 will disable seeded runs!"); }
